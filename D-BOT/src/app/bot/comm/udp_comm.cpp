...
int UDPComm::Send(const JsonDocument& json) {
    if (last_remote_port_ == 0) {
        log_w("UDPComm::Send: no remote address saved yet");
        return -1;
    }
    char buffer[256];
    serializeJson(json, buffer, sizeof(buffer));

    udp_.beginPacket(last_remote_ip_, last_remote_port_);
...
void UDPComm::Loop() {
    ...
    if (packetSize) {
        last_remote_ip_ = udp_.remoteIP();
        last_remote_port_ = udp_.remotePort();

        char buffer[packetSize + 1];
        udp_.read(buffer, packetSize);
        buffer[packetSize] = '\0'; // Ensure null-terminated string
        log_i("received udp msg from %s:%d: %s",
              last_remote_ip_.toString().c_str(), last_remote_port_, buffer);
        JsonDocument jsonDoc;
        deserializeJson(jsonDoc, buffer);

        if (recvCallback_) {
            recvCallback_(jsonDoc, this);
        }
    }
}
