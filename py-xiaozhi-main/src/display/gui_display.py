...
class GuiDisplay(BaseDisplay, QObject, metaclass=CombinedMeta):
    ...
    async def close(self):
        ...
        """
        self._running = False
        if self._dbot_panel and hasattr(self._dbot_panel, "_refresh_timer"):
            self._dbot_panel._refresh_timer.stop()
        # 关闭 D-BOT ACK 监听器
        try:
    ...
    def _load_qml(self):
        ...
        # 设置为主窗口的中央 widget（QML + D-BOT 面板并排）
        layout = QHBoxLayout(self.root)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(0)
        layout.addWidget(self.qml_widget, 1)  # QML 占据剩余空间

        # D-BOT 状态面板（仅 GUI 模式加载，不影响 CLI）
        self._load_dbot_panel(layout)

    def _load_dbot_panel(self, parent_layout):
        """
        加载 D-BOT 状态面板到布局中.
        """
        try:
            from src.views.widgets.dbot_panel import DBotPanel

            self._dbot_panel = DBotPanel(self.root)
            parent_layout.addWidget(self._dbot_panel)
            self.logger.debug("D-BOT 面板已加载")
        except Exception as e:
            self.logger.warning(f"D-BOT 面板加载失败（不影响主功能）: {e}")
