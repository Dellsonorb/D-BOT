...
class ThingManager:
    ...
    async def initialize_iot_devices(self, config):
        ...
        from src.iot.things.dbot import DBotThing

        # 添加基础测试设备
        self.add_thing(Lamp())

        # 添加 D-BOT Thing（若配置禁用则跳过）
        dbot_cfg = None
        try:
            if config is not None:
                dbot_cfg = config.get_config("DBOT", None)
        except Exception as exc:
            logger.warning(f"读取 DBOT 配置失败: {exc}")
            dbot_cfg = None

        if dbot_cfg is None:
            dbot_cfg = {}

        if dbot_cfg.get("ENABLED", True):
            try:
                self.add_thing(DBotThing(dbot_cfg))
                logger.info("D-BOT Thing 已注册")
            except Exception as exc:
                logger.error(f"注册 D-BOT Thing 失败: {exc}", exc_info=True)
        else:
            logger.info("D-BOT Thing 已禁用，跳过注册")
